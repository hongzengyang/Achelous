...
@interface OOXCLiveVC ()<UITextViewDelegate>
...
@property (nonatomic, strong) OOXCLiveModel *model;

@end

@implementation OOXCLiveVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = [UIColor xycColorWithHex:0xF6F6F6];
    
    self.model = [[OOXCLiveModel alloc] init];
...
- (void)clickRightButton {
    if ([NSString xy_isEmpty:self.model.name]) {
        [SVProgressHUD showErrorWithStatus:@"请输入实况名称"];
        return;
    }
    
    if ([NSString xy_isEmpty:self.model.place]) {
        [SVProgressHUD showErrorWithStatus:@"请输入采集地点"];
        return;
    }
